  ...
  const rowExpansionTemplate = (data: Contract) => {
    return (
      <div className='p-3'>
        ...
        <DataTable size='small' dataKey='id' className='!overflow-hidden   !border-none' responsiveLayout='scroll' value={data.DebtClients}>
          <Column
            field='amount'
          ...
          <Column
            ...
            sortable
            body={(data) => (
              <span className={`p-1 rounded-full px-2  ${data.paid ? 'text-green-400' : 'text-red-400'}`}>
                {' '}
                {data.paid ? 'Pagado' : 'Sin pagar'} {data.paid && formatDateDDMMYYYY(data.paidDate.slice(0, 10))}{' '}
              </span>
            )}
            header='Pago'
          ></Column>
                ...
                body={(data) => (
                  <span className={`${data.DebtClients.filter((d: any) => !d.paid).length > 0 ? 'text-red-400' : ''}`}>
                    {data.Client ? (
                      ...
                      <span className='text-red-400 font-light text-sm italic'>Inquilino no encontrado</span>
                    )}
                  </span>
                )}
                header='Inquilino'
