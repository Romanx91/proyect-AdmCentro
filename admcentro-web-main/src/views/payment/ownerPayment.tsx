...
const OwnerPayment = () => {
  ...
  const { OwnerId, PaymentTypeId, month, year, updateAll, total, values, handleInputChange, obs, reset } = useForm({
    OwnerId: null,
    PaymentTypeId: null,
  ...
  const handleChangeOwner = async (e: DropdownChangeEvent) => {
    ...
    try {
      ...
      ownerContracts.data.data.map(async (contract: any) => {
        const docsExpss = http.get<IClientExpensesResponseSimple>(`/owner-expenses?amount=0:ne&ContractId=${contract.id}&include=true`)
        ...
        const paidCurrentMonth = http.get(
          `/payment-clients?ContractId=${contract.id}&month=${month}&year=${year}&paidCurrentMonth=${1}&include=true`
        )
        const hasDebts = http.get(`/debt-clients?ContractId=${contract.id}&paid=${0}&include=true`)

        const [docsExps, docsEvents, docsDebts, padiCurMonth, hasDebtsResponse, eventContracts] = await Promise.all([
          docsExpss,
          docsEventss,
          docsDebtss,
          paidCurrentMonth,
        ...
        let expensess =
          contract.state === 'Finalizado' || isExpired
            ...
            : [
                {
                  amount: roundUp(
                    contract.PriceHistorials.sort((a: IHistorialPrice, b: IHistorialPrice) => a.id - b.id)[
                      contract.PriceHistorials?.length - 1
                    ]?.amount
                  ),
                  description:
                    'ALQUILER ' +
        ...
        setContractRows((prev) => [
          ...prev,
          {
            ...
            eventualityDetails: [...eventContracts.data.data, ...docsEvents.data.data].map((d) => ({
              ...d,
              ContractId: contract.id,
              description:
                d.description +
  ...
  const handleDownloadPdf = async () => {
    ...
    var opt = {
      margin: [0.1, 0.1],
      filename: `${currentPayment.current?.Owner!.fullName}_${currentPayment.current?.month}_${
          ...
          >
            ...
            {upToDate && OwnerId && (
              <div className='text-green-500 dark:text-green-400 text-center my-2'>
                ...
                {Array.from(
                  new Set(
                    lastPayment.map((it) => (typeof it.month === 'number' ? monthsInSpanish[it.month - 1] : it.month ? it.month : month))
                  )
                ).join(',')}
              </div>
            )}
                  ...
                  <div key={l} className='bg-gray-50 shadow-sm border border-gray-300 dark:border-slate-700 dark:bg-slate-900 p-2'>
                    <h2
                      className={`font-semibold text-lg mb-3 ${
                        contract.isExpired ? 'text-red-400' : contract.contract.state == 'Finalizado' ? 'text-yellow-500' : 'text-green-400'
                      } `}
                    >
                      {contract.contract.Property.street} {contract.contract.Property.number} {contract.contract.Property.floor}-
                      {contract.contract.Property.dept} | {contract.contract.Client.fullName}
                      {` | ${contract.contract.state} | ${contract.isExpired ? 'Vencido' : 'Vigente'} | ${formatDateDDMMYYYY(
                        contract.contract.startDate
                      )} - ${formatDateDDMMYYYY(contract.contract.endDate)} | Ajuste : ${contract.contract.adjustmentMonth} meses`}
                    </h2>

                    {upToDate && OwnerId && lastPayment.filter((lp) => lp.ContractId === contract.contract.id).length > 0 && (
                      <div className='text-green-500 dark:text-green-400 text-center my-2'>
                        {/* @ts-ignore */}
                        Ya cobraste {lastPayment.filter((lp) => lp.ContractId === contract.contract.id).length} concepto(s) para el/los
                        mes(es) de{' '}
                        {Array.from(
                          new Set(
                            lastPayment
                              .filter((lp) => lp.ContractId === contract.contract.id)
                              .map((it) => (typeof it.month === 'number' ? monthsInSpanish[it.month - 1] : it.month ? it.month : month))
                          )
                        ).join(',')}
                      </div>
                    )}
                    ...
                    {contract.hasDebts && (
                      <div className={`has-debts border border-dashed p-2  border-red-400 dark:text-red-400 text-center my-2`}>
                        <span className='text-red-500'>El inquilino tiene deudas impagas.</span>
                      ...
                      <div className=''>
                        ...
                        <div className='eventualities-section flex flex-col gap-y-2 gap-x-3'>
                          {contract.expenseDetails.map((evt: any, index: any) => (
                            <div
                              key={evt.id.toString() + evt.createdAt + index + evt.amount + evt.description + evt.ContractId}
                              className='align-items-center flex items-center flex-wrap   border border-gray-300 dark:border-slate-700 p-2'
                            >
                              <Checkbox
                                inputId={evt.id.toString() + evt.createdAt + index + evt.amount + evt.description + evt.ContractId}
                                name='expenseClients'
                                disabled={
                                  lastPayment.filter(
                                    (item: any) => item.description === evt.description && item.ContractId === evt.ContractId
                                  ).length > 0
                                }
                                value={evt}
                                onChange={onExpensesClienteChange}
                                checked={selectedExpensesClient.some(
                                  (item: any) => item.id === evt.id && item.ContractId === evt.ContractId
                                )}
                              />
                              <label
                                htmlFor={evt.id.toString() + evt.createdAt + index + evt.amount + evt.description + evt.ContractId}
                                className={` ml-2  ${
                                  lastPayment.filter(
                                    (item: any) => item.description === evt.description && item.ContractId === evt.ContractId
                                  ).length > 0
                      ...
                      <div className='my-4'>
                        ...
                        <div className='eventualities-section  flex flex-col gap-y-2  gap-x-3'>
                          {contract.eventualityDetails.map((evt: any, index: any) => (
                            <div
                              key={evt.updatedAt + evt.description + index + evt.ownerAmount + evt.description + evt.ContractId}
                              className='align-items-center flex cursor-pointe items-center flex-wrap   border border-gray-300 dark:border-slate-700 p-2'
                            >
                              <Checkbox
                                inputId={evt.updatedAt + evt.description + index + evt.ownerAmount + evt.description + evt.ContractId}
                                value={evt}
                                name='eventuality'
                              ...
                              />
                              <label
                                htmlFor={evt.updatedAt + evt.description + index + evt.ownerAmount + evt.description + evt.ContractId}
                      ...
                      <div className='my-4'>
                        ...
                        <div className='eventualities-section flex flex-col gap-y-2 gap-x-3'>
                          {contract.debts.map((evt: any, index: any) => (
                            <div
                              key={evt.updatedAt + evt.id + index + evt.amount + evt.description + evt.ContractId}
                            ...
                            >
                              <Checkbox
                                inputId={evt.updatedAt + evt.id + index + evt.amount + evt.description + evt.ContractId}
                                value={evt}
                                name='debtsProperties'
                                onChange={onDebtsChanges}
                                checked={selectedDebts.some((item: any) => item.id === evt.id && item.ContractId === evt.ContractId)}
                              />
                              <label
                                htmlFor={evt.updatedAt + evt.id + index + evt.amount + evt.description + evt.ContractId}
                                className='ml-2 cursor-pointer'
                              >
                                <span className=''>
                                  ${roundUp(evt.amount)} {evt.description}
            ...
            <FieldsetGroup>
              <FieldsetGroup className='w-full sm:w-[50%]'>
                <fieldset className=''>
                  ...
                  <input placeholder='1234' type='number' disabled={true} value={eventTotal.current} onChange={() => {}} />
                </fieldset>
                <fieldset className=''>
      ...
      >
        ...
        <div className='!shadow-none rounded-lg  border-gray-200  overflow-y-auto max-h-[450px] mt-3'>
          <div id='pdf-download' className='flex gap-x-2 '>
            {[1, 2].map((pdf, index) => (
              <div key={index} className='flex  justify-between flex-col border border-gray-200  p-1  min-h-[750px] w-[500px]'>
                    ...
                    <div key={index} className=' border border-gray-200  p-2'>
                      ...
                      {con[0].contract && (
                        <span className='vencimiento-contrato text-xs text-yellow-500'>
                          Vigencia contrato : {formatDateDDMMYYYY(con[0].contract?.startDate)} |{' '}
          ...
          <div className='flex gap-x-2 sticky bottom-0 bg-white px-4'>
            ...
            <button className='btn gradient  !my-4' disabled={loadingPdf} onClick={handleDownloadPdf}>
