...
const ReceiptDataTable = ({ loadingPdf, data, isOwner = false, closePrintPdfModal, handleDownloadPdf }: Props) => {
  const getTotal = data.map((p) => (p.paidTotal > 0 ? p.paidTotal : p.total)).reduce((a, b) => a + b, 0)
  // get total for payment of type cash
  ...
  return (
    <div className='shadow-none border-0  border-gray-200  text-slate-900 max-h-[450px]  overflow-y-auto'>
      <div id='planillaCobros' className={`flex gap-x-2 `}>
          ...
          <div className='mt-2 flex-1 flex flex-col justify-between'>
            <div className='not-paid-contracts flex flex-col  gap-y-1d divide-y-[0.1px] divide-slate-300 '>
              <div className='not-paid-contract-item-body flex  gap-x-1 p-2'>
                ...
                <div className={' font-medium  flex items-center justify-center text-center  ' + w}>PROPIEDAD</div>
                <div className={' font-medium  flex items-center justify-center text-center  ' + w}>CARPETA</div>
                ...
                return (
                  <div className=' text-slate-900 p-2' key={index}>
                    <div className=' flex  gap-x-1 '>
                      <div className={'flex items-center justify-center text-center ' + w}>
                        {p.Contract.Property.street +
                          ' ' +
                          p.Contract.Property.number +
                          ' ' +
                          p.Contract.Property.floor +
                          ' ' +
                          p.Contract.Property.dept}
                      </div>
                      <div className={'flex items-center justify-center text-center ' + w}>{p.Contract.Property.folderNumber}</div>
